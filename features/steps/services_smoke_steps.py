...
def step_submit_service(context):
    ...
    context.loop.run_until_complete(services_page.submit_service('Smoke Service'))

@then('the smoke service request should be submitted successfully')
def step_smoke_request_submitted(context):
    services_page = ServicesPage(context.page)
